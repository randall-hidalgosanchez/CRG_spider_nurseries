...
df |> ggplot() +
  geom_line(aes(x = date, y = mean_area, color = site), linewidth = 1) +
  geom_point(aes(x = date, y = mean_area, color = site), size = 2) +
...
df |> ggplot() +
  ...
  theme_classic() +
  scale_color_manual(values = paleta,
                     labels = c("Punta Cacique", "Playa Blanca")) +
  scale_x_date(date_labels = "%b %Y", date_breaks = "6 months") +
  theme(legend.position = "top",
        legend.title = element_blank(),
        plot.title = element_text(hjust = 0.5, size = 16),
...
data <- big %>%
  ...
  summarise(annual_cover = last(area.cm2)) # Use the last measurement in each year
...
big %>%
  ...
  group_by(year) %>%
  summarise(annual_cover = last(area.cm2)) %>%
  arrange(year) %>%

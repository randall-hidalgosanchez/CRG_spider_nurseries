...
Remove data analysed by Patricia, Andrea C. and Carmen. Patricia's data were recorded using wrong scales, whilst Andrea and Carmen's data are the same as Chiara's. I don't even know what Shantall was doing...

```{r}
levels(final_ds2$data_entry_person)

final_ds2 <- final_ds2 %>% 
  filter(!data_entry_person %in% c("Patricia", "Andrea C.", 
                                   "Carmen", "Shantall")) %>% 
  droplevels()
```

Now, we want to keep data from January 2020 to June 2026 (indico 2026 porque quiero mantener todos los datos) 
...
final_ds2 <- final_ds2 %>% 
    filter(between(date, as_date("2021-01-01"), as_date("2026-06-01")))
```
